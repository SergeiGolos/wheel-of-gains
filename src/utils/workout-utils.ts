...
export const saveWorkoutsToStorage = (
  workouts: Workout[],
  storageKey?: string,
) => {
  try {
...
export const createWorkoutUrl = (name: string) =>
  `https://www.google.com/search?q=${encodeURIComponent(name + " workout")}`;
...
export const initialMasterWorkouts: Workout[] = initialWorkoutStrings.map(
  (str) => {
...
export const addSpinResult = (
  workout: Workout,
  existingHistory: SpinResult[],
): SpinResult[] => {
