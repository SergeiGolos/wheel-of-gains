...
const mockValidationFunctions = {
  ...
  isWorkoutNameUnique: (name: string, workouts: any[], excludeId?: string): boolean => {
    const trimmedName = name.trim().toLowerCase();
    return !workouts.some(
      (w) => w.id !== excludeId && w.name.toLowerCase() === trimmedName,
    );
  },
  validateNewWorkoutForm: (name: string, multiplier: number, workouts: any[]) => {
    const errors = {
      name: false,
      multiplier: false,
      duplicate: false,
    ...
    errors.duplicate = !mockValidationFunctions.isWorkoutNameUnique(name, workouts);
...
const meta: Meta = {
  ...
  argTypes: {
    ...
    testScenario: {
      control: { type: "select" },
      options: ["valid", "empty_name", "invalid_multiplier", "duplicate_name", "all_errors"],
...
const renderValidationTest = (args: any) => {
  const { workoutName = "Test Workout", multiplier = 1, testScenario = "valid" } = args;
  ...
  const multiplierValid = mockValidationFunctions.validateMultiplier(testMultiplier);
  const nameUnique = mockValidationFunctions.isWorkoutNameUnique(testName, mockWorkouts);
  const formValidation = mockValidationFunctions.validateNewWorkoutForm(testName, testMultiplier, mockWorkouts);
      ...
      <div class="p-4 rounded-lg ${formValidation.isValid ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}">
        ...
        <div class="text-sm ${formValidation.isValid ? 'text-green-600' : 'text-red-600'}">
          ${formValidation.isValid ? 
            ...
            `Validation errors: ${Object.keys(formValidation.errors).filter(key => formValidation.errors[key]).join(', ')}`
          }
        </div>
      </div>
...
export const InteractiveValidation: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "Interactive validation testing - use controls to test different inputs.",
      },
    },
  },
};